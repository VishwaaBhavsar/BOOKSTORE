...
function Navbar()
{
  const [theme,useTheme]=useState(localStorage.getItem("theme")? localStorage.getItem("theme"):"light")
  const element=document.do
...
useEffect(()=>{
 ...
 return ()=>{
  ...
  return ( 
    <>
    <div className={`max-w-screen-2xl container mx-auto md:px-20 px-4 fixed top-0 right-0 left-0 ${sticky? "sticky-navbar shadow-md bg-base-100 duration-300 transition-all ease-in-out":""}`}>
    <div className="navbar bg-base-100 w-full shadow-accent-content">
  ...
  <div className="flex items-center gap-2 px-3 py-1 md:block hidden">
...
<label className="swap swap-rotate">
  ...
  <div className> 
    <a className="bg-black text-white px-3 py-2 rounded-md hover:bg-slate-300 duration-300 cursor-pointer">Login</a>
  </div>
</div>
</div>
    </div>
